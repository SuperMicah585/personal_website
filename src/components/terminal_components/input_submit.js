...
const InputAndSubmit = ({ windowWidth, dragTriggerValue, dragInputValue, isloadingCallBack, loadingValueCallBack, terminalTextCallBack, isInputHovered }) => {
  ...
  const handleInputScroll = () => {
    if (inputScrollRef.current && scrollableDivRef.current) {
      // Sync the scroll position of both elements
      scrollableDivRef.current.scrollLeft = inputScrollRef.current.scrollLeft;
    }
  };
  ...
  const tailwindColors = ['text-slate-300 font-bold', 'text-blue-500 font-bold', 'text-blue-300 font-bold'];
